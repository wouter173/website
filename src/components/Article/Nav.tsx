...
const Nav = ({ metadataTitleRef, slug }: Props) => {
	...
	return (
        ...
        sticky top-0 z-20 grid max-w-full gap-5 bg-bermuda 
        bg-opacity-95 px-2 py-4 backdrop-blur-lg backdrop-filter transition-all
      `}
		...
		>
			...
			<span
				className={`${
					y > (metadataTitleRef.current?.offsetTop || 1) ? 'opacity-100' : 'opacity-0'
